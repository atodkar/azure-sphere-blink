...
static int echoFd = -1;
static int triggerFd = -1;
static int ultrasoundTimerFd = -1;
...
///     Sends a message to the IoT Hub.
/// </summary>
static void UltrasoundUpdateHandler(event_data_t *eventData)
{
	if (ConsumeTimerFdEvent(ultrasoundTimerFd) != 0) {
		terminationRequired = true;
		return;
	}
	...
	if (connectedToIoTHub) {
		...
		int result = GPIO_SetValue(triggerFd, GPIO_Value_Low);
		if (result != 0) {
			Log_Debug("ERROR: Cannot write GPIO.\n");
		}
		...
		result = GPIO_SetValue(triggerFd, GPIO_Value_High);
		if (result != 0) {
			Log_Debug("ERROR: Cannot write second GPIO.\n");
		}
		...
		result = GPIO_SetValue(triggerFd, GPIO_Value_Low);
		if (result != 0) {
			Log_Debug("ERROR: Cannot write third GPIO.\n");
		}

		result = GPIO_SetValue(echoFd, GPIO_Value_High);
		ts.tv_sec = 1 / 1000;
		ts.tv_nsec = (1 % 1000) * 1000000;
		nanosleep(&ts, NULL);
		result = GPIO_SetValue(echoFd, GPIO_Value_Low);

		GPIO_Value_Type value = GPIO_Value_Low;
		do {
			result = GPIO_GetValue(echoFd, &value);
		} while (value == GPIO_Value_Low);

		Log_Debug("GPIO value is: %d .\n", value);
		...
		do {
			result = GPIO_GetValue(echoFd, &value);
		} while (value == GPIO_Value_High);

		struct timeval tv_stop = { 0,0 };
		gettimeofday(&tv_stop, NULL);

		int milliSeconds = (tv_stop.tv_sec - tv_start.tv_sec) * 1000000 + (tv_stop.tv_usec - tv_start.tv_usec);

		Log_Debug("Value read from Echo %f cm:.\n", milliSeconds * 0.034 / 2);
		Log_Debug("Value read from Echo %f inch:.\n", milliSeconds * 0.0133 / 2);

		if (result != 0) {
			Log_Debug("ERROR: Cannot write third GPIO.\n");
		}

		sprintf(str, "{\"ultrasound\":\"true\", \"distance\": %d}", n);
		AzureIoT_SendMessage(str);
		free(str);
	}
	else {
		Log_Debug("WARNING: Cannot send message: not connected to the IoT Hub.\n");
...
static event_data_t azureIotUltrasoundData = {.eventHandler = &UltrasoundUpdateHandler };
...
static int InitPeripheralsAndHandlers(void)
{
	...
	// Open GPIO pin 0
	echoFd = GPIO_OpenAsOutput(MT3620_GPIO0, GPIO_OutputMode_OpenSource, GPIO_Value_High);
	if (echoFd < 0) {
		Log_Debug("ERROR: Could not open GPIO0: %s (%d).\n", strerror(errno), errno);
		return -1;
	}

	// Open GPIO pin 0
	triggerFd = GPIO_OpenAsOutput(MT3620_GPIO1, GPIO_OutputMode_OpenSource, GPIO_Value_High);
	if (triggerFd < 0) {
		Log_Debug("ERROR: Could not open GPIO1: %s (%d).\n", strerror(errno), errno);
		return -1;
	}
	...
	// Set up a timer for Azure IoT Ultrasound execution.
	static struct timespec ultrasoundPeriod = { 1, 0 };
	ultrasoundTimerFd =
		CreateTimerFdAndAddToEpoll(epollFd, &ultrasoundPeriod, &azureIotUltrasoundData, EPOLLIN);
	if (ultrasoundTimerFd < 0) {
		return -1;
	}
...
static void ClosePeripheralsAndHandlers(void)
{
	...
	CloseFdAndPrintError(ultrasoundTimerFd, "UltrasoundTimer");
